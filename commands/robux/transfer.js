...
module.exports = {
  ...
  examples: ['transfer AhmedBoza0 (25)'],
  cooldown: 5,
  dm: true,
  args: true,
  async execute(message, args, client) { 
   ...
   // if (!guild) return message.lineReplyNoMention(replys.noserver);
    ...
    await noblox.getIdFromUsername(args[0].toString()).then(async (clientId) => {
      if (isNaN(args[1].toString()) || parseInt(args[1].toString()) != args[1].toString() || parseInt(args[1].toString()) < 1) return message.lineReplyNoMention();
      ...
      await noblox.setCookie(data2.system.cookies).then(async (user) => {
       ...
       // message.guild.channels.cache.get("918074014616936449").send(attach);
