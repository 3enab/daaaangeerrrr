...
module.exports = {
  ...
  async execute(message, args, client) {
    ...
    let embed = new MessageEmbed().setColor(client.color).setDescription(replys.transfer.replace(/\{price}/g, `${price}`).replace(/\{owner}/g, `${owner}`).replace(/\{id}/g, `${owner.id}`));
    var msg = await message.lineReplyNoMention('', { embed: embed });
    price = Math.floor(price - price * (5 / 100));
    if (price <= 0) price = 1;
    
    let pay = await message.channel.createMessageCollector(user => (user.author.id == `282859044593598464` || user.author.id == `567703512763334685`) && (user.content.includes(message.author.username) && user.content.includes(`$${price}`) && (user.content.includes(`<@!${owner.id}>`) || user.content.includes(`<@${owner.id}>`))), { time: 60000 * 2 });
