...
module.exports = {
  ...
  async execute(message, args, client) { 
    ...
    if(!limitUsers) return message.channel.send('Please enter a limit.');
    if(!gifted) return message.channel.send('Please enter a gifted robux.');

    if (isNaN(limitUsers)) return message.channel.send('Limit users must be number');
    if (isNaN(gifted)) return message.channel.send('Gifted robux must be number');
    ...
    if (data.system.giftsCode.map( e => e.code ).includes(code)) return message.channel.send("**That code already added!**");
