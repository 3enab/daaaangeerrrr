...
client.replys = require("./src/languages/" +
  language.substr(0, 2).toLowerCase());
client.color = color;
client.events = new EventEmitter();
client.database = {
  guildsData: new DataBaseManager(dataBaseInfo, {
    name: "guildsData",
    default: guildsData,
  }),
  usersData: new DataBaseManager(dataBaseInfo, {
    name: "usersData",
    default: usersData,
  }),
};
...
for (let folder of readdirSync("commands").filter(
  (folder) => !folder.includes(".")
)) {
  for (let file of readdirSync("commands/" + folder).filter((f) =>
    f.endsWith(".js")
  )) {
